...
int Rasgos_Persona_es_de_riesgo(RasgosDeRiesgo *rasgos, Person* persona) {
    ...
    return esta_prendido_en_mascara(&(rasgos->mascaraCaracteristicasEspeciales), car->caracteristicasEspeciales) ||
...
int Rasgos_Aniadir_caracteristica_especial(RasgosDeRiesgo* rasgos, enum caracteristicas_especiales nueva_caracteristica) {
    if (nueva_caracteristica > MAX_CARACTERISTICA_ESPECIAL)
        return ARGUMENTO_INVALIDO;
    aniadir_a_mascara(&(rasgos->mascaraCaracteristicasEspeciales), nueva_caracteristica);
    return CAMBIO_OK;

}

int Rasgos_Remover_caracteristica_especial(RasgosDeRiesgo* rasgos, enum caracteristicas_especiales caracteristica) {
    if (caracteristica > MAX_CARACTERISTICA_ESPECIAL)
        return ARGUMENTO_INVALIDO;
    remover_de_mascara(&(rasgos->mascaraCaracteristicasEspeciales), caracteristica);
    return CAMBIO_OK;
}
